...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/04/29 16:22:41 by vdaviot           #+#    #+#             */
/*   Updated: 2016/05/05 17:41:18 by gjensen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_sh.h"

static void			set_time(t_hist *histo)
{
	char			*tmp;
...
void				ft_init_history(void)
{
	...
	i = -1;
	if (!(tb = (char **)malloc(sizeof(char*) * (HISTORY_SIZE + 1))))
		return ;
...
void				ft_add_history(char *str)
{
	...
	while ((count_lines_file(g_history_file) > HISTORY_SIZE))
		remove_excess_line(g_history_file);
	ptr = g_history;
	decrement_index(ptr);
	ft_strdel(&tmp);
}
